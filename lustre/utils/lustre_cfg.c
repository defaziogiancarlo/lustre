...
int jt_lcfg_listparam(int argc, char **argv)
{
	...
	for (i = index; i < argc; i++) {
		...
		rc2 = param_display(&popt, path, NULL, LIST_PARAM, stdout);
		if (rc2 < 0) {
...
int jt_nodemap_info(int argc, char **argv)
{
	...
	if (argc == 1 || strcmp("list", argv[1]) == 0) {
		popt.po_only_path = 1;
		popt.po_only_dir = 1;
		rc = param_display(&popt, "nodemap/*", NULL, LIST_PARAM, stdout);
		...
		rc = param_display(&popt, pattern, NULL, LIST_PARAM, stdout);
		if (rc == -ESRCH)
...
int jt_lcfg_setparam(int argc, char **argv)
{
	...
	for (i = index; i < argc; i++) {
		...
		rc2 = param_display(&popt, path, value, SET_PARAM, stdout);
		if (rc == 0)
			rc = rc2;
	}
