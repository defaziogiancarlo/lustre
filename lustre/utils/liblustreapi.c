 ...
 *				parameter path
 * \param[in]	  popt		set/get param options
 * \param[in,out] output_fp     output written to file
 *
 * \retval 0 on success.
 * \retval -errno on error.
 */
...
read_param(const char *path, const char *param_name,
	   struct param_opts *popt, FILE* output_fp)
{
	...
	/* Read the contents of file to output_fp (stdout by default) */
	...
	while (1) {
		...
		if (display_path) {
			...
			fprintf(output_fp, "%s=%s", param_name, longbuf ? "\n" : buf);
		...
		if (fwrite(buf, 1, count, output_fp) != count) {
			rc = -errno;
			if (output_fp == stdout) {
				fprintf(stderr,
...
int
llapi_param_simple_access(char *pattern,  char *value,
			 enum parameter_operation mode, FILE *output_fp)
{
	return llapi_param_access(NULL, pattern, value, mode, output_fp);
}

int
llapi_param_access(struct param_opts *popt_p, char *pattern, char *value,
	      enum parameter_operation mode, FILE *output_fp)
{
	int dup_count = 0;
	char **dup_cache;
	...
	/* print to stdout if output_fp not given */
        output_fp = output_fp ? output_fp : stdout;
	...
	for (i = 0; i < paths.gl_pathc; i++) {
		...
		switch (mode) {
		case GET_PARAM:
			/* Read the contents of file to ouput_fp */
		...
		rc2 = llapi_param_access(popt, pathname, value, mode, output_fp);
		if (rc2 != 0 && rc2 != -ENOENT) {
