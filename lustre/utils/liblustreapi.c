 ...
 * \param[in,out] output_fp     where to send the output
 *
 * \retval number of bytes written on success.
...
int
llapi_param_access(struct param_opts *popt, char *pattern, char *value,
	      enum parameter_operation mode, FILE *output_fp)
{
	...
	if (!output_fp) {
		rc = -EINVAL;
		fprintf(stderr, "error: %s: output_fp: %s\n",
			opname, strerror(-rc));
		return rc;
	}
